...
class PPM_Block(nn.Module):
    ...
    def forward(self, x):
        conv1 = self.act(self.conv(self.pool(x, x.shape[-3] // 4)))
        conv2 = self.act(self.conv(self.pool(x, x.shape[-3] // 8)))
        conv3 = self.act(self.conv(self.pool(x, x.shape[-3] // 16)))
        conv4 = self.act(self.conv(self.pool(x, x.shape[-3] // 32)))
...
class conv_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.dpconv = nn.Sequential(
            ...
            # DepthwiseSeparableConv(ch_in, ch_out, kernel_size=3, padding=1, kernels_per_layer=1),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),
        )
        # self.dpconv_only = DepthwiseSeparableConv(ch_out, ch_out, kernel_size=3, padding=1)
        self.dpconv_only = nn.Conv2d(ch_out, ch_out, kernel_size=3, padding=1)

        self.SE = SE_Block(ch_out)
        # self.SE = CBAM(ch_out, reduction=8)
        self.BN = nn.BatchNorm2d(ch_out)
        self.act = nn.ReLU(inplace=True)

    def forward(self, x):
        x1 = self.dpconv(x)
        x2 = self.dpconv_only(x1)
        x = self.SE(x2)
...
class DcReSPnet(nn.Module):
    # def __init__(self, in_channels=1):
    def __init__(self, args):
        in_channels = 4
        super(DcReSPnet, self).__init__()
        ...
        self.out1 = nn.Conv2d(32, 3, kernel_size=1, stride=1, padding=0)
        self.ppm = PPM_Block(512)

    def forward(self, x):
        # encoding path     x (18,4,160,160)

        x1 = self.in1(x)  # x1 - (512,512,32)   (18,32,160,160)

        mp = self.Maxpool(x1)
        ap = self.Avgpool(x1)
        p1 = torch.add(mp, ap)  # p1 - (256,256,32)
        x2 = self.Conv1(p1)  # x2 - (256,256,64)   (18,64,80,80)

        mp = self.Maxpool(x2)
        ap = self.Avgpool(x2)
        p2 = torch.add(mp, ap)  # p2 - (128,128,64)
        x3 = self.Conv2(p2)  # x3 - (128,128,128)  (18,128,40,40)

        mp = self.Maxpool(x3)
        ap = self.Avgpool(x3)
        p3 = torch.add(mp, ap)  # p3 - (64,64,128)
        x4 = self.Conv3(p3)  # x4 - (64,64,256)   (18,256,20,20)

        mp = self.Maxpool(x4)
        ap = self.Avgpool(x4)
        p4 = torch.add(mp, ap)  # p4 - (32,32,256)
        x5 = self.Conv4(p4)  # x4 - (32,32,512)   (18,512,10,10)

        x5 = self.ppm(x5)  # x5 - (32,32,512)

        # decoding & concat path
        d4 = self.Up4(x5)  # (18,256,20,20)

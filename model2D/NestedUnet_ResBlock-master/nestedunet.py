...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.double_conv = nn.Sequential(
            ...
            # DefC(in_channels, out_channels, kernel_size=3, padding=1),

            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),

            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1),
            # DefC(out_channels, out_channels, kernel_size=3, padding=1),
...
class NestedUnet(nn.Module):
    def __init__(self, args):
        ...
        if self.args.deepsupervision:
            self.final1 = nn.Conv2d(nb_filter[0], 3, kernel_size=1)
            self.final2 = nn.Conv2d(nb_filter[0], 3, kernel_size=1)
            self.final3 = nn.Conv2d(nb_filter[0], 3, kernel_size=1)
            self.final4 = nn.Conv2d(nb_filter[0], 3, kernel_size=1)
        else:

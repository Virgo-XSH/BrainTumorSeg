...
import param
IMG_PATH = glob(param.IMG_PATH_TEST)
MASK_PATH = glob(param.MASK_PATH_TEST)
...
def parse_args():
    ...
    parser.add_argument('--name', default='Brats_MCNet_woDS',
                        help='model name')
...
def main():
    ...
    if not os.path.exists('output4/%s' %args.name):
        os.makedirs('output4/%s' %args.name)
    print('Config -----')
            ...
            for mynum, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
                ...
                for i in range(output.shape[0]):
                    if (startFlag == 1):#第一个块的处理
                        ...
                        OnePeople = np.zeros([155, 192, 192], dtype=np.uint8)
                        OneMask = np.zeros([155, 192, 192], dtype=np.uint8)
                        # 创建三个全黑的三维矩阵，分别用于预测出来的WT、TC、ET分块的拼接
                        OneWT = np.zeros([155, 192, 192], dtype=np.uint8)
                        OneTC = np.zeros([155, 192, 192], dtype=np.uint8)
                        OneET = np.zeros([155, 192, 192], dtype=np.uint8)
                        # 创建三个全黑的三维矩阵，分别用于真实的WT、TC、ET分块的拼接
                        OneWTMask = np.zeros([155, 192, 192], dtype=np.uint8)
                        OneTCMask = np.zeros([155, 192, 192], dtype=np.uint8)
                        OneETMask = np.zeros([155, 192, 192], dtype=np.uint8)
                        # 处理预测后的分块
                        # (2, 3, 192, 192) output
                        # 预测切片的拼接
                    ...
                    if (NameNow != LastName):
                        ...
                        SavePeoPle[:, 24:216, 24:216] = OnePeople[:, :, :]

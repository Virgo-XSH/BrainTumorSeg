...
arch_names = list(MCNet.__dict__.keys())
...
def parse_args():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='MCNet',
                        choices=arch_names,
...
def train(args, train_loader, model, criterion, optimizer, epoch, scheduler=None,val_loss=None):
    losses = AverageMeter()
    ious = AverageMeter()
    dices = AverageMeter()
    model.train()
...
def main():
    ...
    model = MCNet.__dict__[args.arch](args)
    ...
    if args.optimizer == 'Adam':
        ...
        # schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.1)

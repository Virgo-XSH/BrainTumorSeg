...
arch_names = list(Res2Net_UNet.__dict__.keys())
loss_names = list(losses.__dict__.keys())
loss_names.append('BCEWithLogitsLoss')
import sys

sys.path.append("/home/featurize/work/BrainMriSeg")
import param

IMG_PATH = glob(param.IMG_PATH)
MASK_PATH = glob(param.MASK_PATH)
...
def parse_args():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='Res2_UNet',
                        choices=arch_names,
    ...
    parser.add_argument('--weight-decay', default=1e-4, type=float,
                        help='weight decay')
    parser.add_argument('--nesterov', default=0.9, type=str2bool,
                        help='nesterov')
...
def train(args, train_loader, model, criterion, optimizer, epoch, scheduler=None):
    ...
    for i, (input, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if args.deepsupervision:
            ...
            dice = dice_coef(output[-1], target)
        else:
            output = model(input)
            loss = criterion(output, target)
...
def main():
    ...
    train_img_paths, val_img_paths, train_mask_paths, val_mask_paths = \
        train_test_split(img_paths, mask_paths, test_size=param.test_size, random_state=41)
    print("train_num:%s" % str(len(train_img_paths)))
    print("val_num:%s" % str(len(val_img_paths)))

    # create model
    print("=> creating model %s" % args.arch)
    model = Res2Net_UNet.__dict__[args.arch](args)
    model = model.cuda()
    lossmin = np.inf
    ...
    if args.optimizer == 'Adam':
        ...
        schedule = torch.optim.lr_scheduler.ExponentialLR(optimizer,  gamma=0.7)
    ...
    earlyStop = EarlyStopping(patience=args.early_stop, verbose=True,
                              path=sys.path[0] + '/models/%s/model.pth' % args.name,
                              val_loss_min=lossmin)  # val_loss_min=lossmin
    ...
    writer.close()
    plt.plot(np.arange(0, 0 + len(lr_list)), lr_list, label="train lr")
    plt.xlabel('epoch')
    plt.ylabel("lr")
    plt.title('Model lr')
    plt.show()

def ReadLossMin(fpath, columnName):
    df_flow = pd.read_csv(fpath)
    return df_flow[columnName].min()

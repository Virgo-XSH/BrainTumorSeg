...
def parse_args():
    ...
    parser.add_argument('--name', default='Brats_Res2_UNet_woDS_不带注意力',
                        help='model name')
...
def main():
    ...
    model = unet5.__dict__[args.arch](args)
    ...
    if val_args.mode == "GetPicture":
        ...
        with warnings.catch_warnings():
            ...
            with torch.no_grad():
                #input的shape为torch.Size([18, 4, 192, 192])   输入18张npy, 每张npy(4x192x192, 含4个模态信息)
                #target的shape为torch.Size([18, 3, 192, 192])   输出18张npy, 每张npy(3x192x192, 含3个分类标签)
                for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    #print("output_shape:%s"%str(output.shape)) # (18,3,192,192)  18张3分类标签192x192
                    ...
                    for i in range(output.shape[0]):
                        ...
                        rgbName = rgbName  + ".png"
                        rgbPic = np.zeros([192, 192, 3], dtype=np.uint8)
                        #拼接每张图的各通道像素值
                        for idx in range(output.shape[2]):
                            for idy in range(output.shape[3]):
                                ...
                                if output[i,2,idx,idy] > 0.5:
                                    ...
                                    rgbPic[idx, idy, 2] = 0
                        imsave('output3/%s/'%args.name + rgbName,rgbPic)
    ...
    if val_args.mode == "Calculate":
        ...
        maskPath = glob("output3/%s/" % args.name + "GT/*.png")
        pbPath = glob("output3/%s/" % args.name + "*.png")
        if len(maskPath) == 0:
            print("请先生成图片!")
            return
